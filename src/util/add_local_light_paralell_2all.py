...
def add_parallel_light_2all(DATA_DIR, TARGET_DIR, light_position=None, direction=None, max_brightness=255, min_brightness=0,
    ...
    if os.path.exists(TARGET_DIR):
        shutil.rmtree(TARGET_DIR)  # Delete an entire directory tree
    if not os.path.exists(TARGET_DIR):
        os.makedirs(TARGET_DIR)	# Make a directory
    ...
    for file in files:
        ...
        if not os.path.exists(TARGET_DIR+'/'):
            os.makedirs(TARGET_DIR+'/')	# Make a directory
        cv2.imwrite(TARGET_DIR+'/'+os.path.basename(file), frame)
        print('Save', TARGET_DIR+'/'+os.path.basename(file))
    ...
    if os.path.isdir(DATA_DIR):
        ...
        for file in files:
            if os.path.isdir(DATA_DIR +'/'+file):
                ...
                add_parallel_light_2all(DATA_DIR, TARGET_DIR)
                break
